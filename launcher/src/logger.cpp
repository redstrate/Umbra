...
public:
    ...
    void initialize()
    {
        ...
        auto existingLogEntries = logDirectory.entryList({QStringLiteral("umbra.*.log")});
        ...
        for (const auto &entry : existingLogEntries) {
            ...
            if (info.exists()) {
                ...
                const auto &newName = logDirectory.absoluteFilePath(QStringLiteral("umbra.%1.log").arg(QString::number(index + 1)));
