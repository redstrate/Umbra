...
Kirigami.ApplicationWindow {
    ...
    function checkSetup(): void {
        ...
        if (!LauncherCore.currentProfile.isGameInstalled) {
            ...
            pageStack.push(Qt.createComponent("zone.xiv.umbra", "SetupPage"), {
            ...
            if (LauncherCore.accountManager.hasAnyAccounts()) {
                ...
                pageStack.push(Qt.createComponent("zone.xiv.umbra", "AccountSetup"), {
                    profile: LauncherCore.currentProfile
                });
            } else {
                // They have no pre-existing accounts, and should be shoved into the new account page
                pageStack.push(Qt.createComponent("zone.xiv.umbra", "AddSquareEnix"), {
                    profile: LauncherCore.currentProfile
                });
            }
        } else {
            if (LauncherCore.autoLoginProfile && !checkedAutoLogin) {
                pageStack.push(Qt.createComponent("zone.xiv.umbra", "AutoLoginPage"))
                checkedAutoLogin = true;
            } else {
                pageStack.push(Qt.createComponent("zone.xiv.umbra", "MainPage"))
            }
        }
    }
